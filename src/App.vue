...
export default {
  ...
  setup() {
    ...
    emitter.on("addStatXp", (info) => {
      addXpToStat(info.xp, info.stat);
    });
    ...
    const addXpToStat = async (newXp, stat) => {
      const { updateDoc } = useDocument(
        "users/" + userInfo.value.id + "/attributes",
        stat.id
      );
       ...
       //console.log(xp);
        ...
        if (gainLevel != 0) {
          toast.add({
            severity: "success",
            summary: `You are now level ${level} in ${stat.name}`,
            life: 3000,
          });
        }
